# D-optimization of sparse sensor placement
import argparse
import os
...
def main():
    ...
    for i in range(p):
        print(
            "  > ENSEMBLE DHMODE DETERMINED OBS LOCATION IS: %d, %d."
            % (ssp_loc[i][0], ssp_loc[i][1])
        )
    filename_max_coord = os.path.join(
        ".", "max_coord_dhmode_%d.tbl" % (args.lr))
    ...
    for obs_num in range(p):
        ...
        if obs_num + ep < r:
            for obs_loc in range(n):
                CC[-1] = u[obs_loc]
                obj[obs_loc] = np.linalg.det(CC @ CC.T)
        else:
            for obs_loc in range(n):
                CC[-1] = u[obs_loc]
                obj[obs_loc] = np.linalg.det(CC.T @ CC)
